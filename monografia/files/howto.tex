...
No método %__createKeyboardLayout
 do módulo calcGui é feita a adição do botões na tela. Escolha uma posição e adicione o 
botão através de %self.__addGridButton(grid, "INV /", 1, 5)
.

Agora, criar a ação relativa ao botão no método %keyboardButtonClicked(self, text)
do controller: 
%        elif text == "INV /":
%            self.__model.invDiv()
